...
export default async function EstacionamientoPage() {
  ...
  const occupiedSpaces = parkingSpaces.filter((space: { status: string }) => space.status === 'occupied');
  const availableSpaces = parkingSpaces.filter((space: { status: string }) => space.status === 'available');
  
  return (
    <div className="container mx-auto p-6">
      <div className="mb-6">
        <h1 className="text-3xl font-bold">Estacionamiento</h1>
        <p className="text-muted-foreground">Gestión de espacios de estacionamiento y actividades</p>
      </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              {occupiedSpaces.slice(0, 5).map((space: { id: string; code: string; vehiclePlate?: string }) => (
                <div key={space.id} className="flex items-center space-x-4">
                  <Car className="h-4 w-4 text-red-500" />
                  <div className="flex-1">
                    <p className="text-sm font-medium">Espacio {space.code}</p>
                    <p className="text-xs text-muted-foreground">
                      {space.vehiclePlate ? `${space.vehiclePlate}` : 'Información no disponible'}
                    </p>
                  </div>
            ...
            <div className="space-y-4">
              {incidents.slice(0, 5).map((incident) => (
                <div key={incident.id} className="flex items-center space-x-4">
                  ...
                  <Badge variant={incident.status === 'resuelto' ? 'default' : 'destructive'}>
                    {incident.status}
                  </Badge>
                </div>
              ))}
