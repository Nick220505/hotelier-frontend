...
export function EmployeePerformanceChart({ employees }: EmployeePerformanceChartProps) {
  ...
  const performanceData = employees
    ...
    .map(emp => {
      ...
      return {
        ...
        efficiency: completionRate >= 80 ? 'Excelente' : completionRate >= 60 ? 'Bueno' : 'Regular',
      };
    })
  ...
  const CustomTooltip = ({ active, payload }: { active?: boolean; payload?: Array<{ payload: { name: string; completionRate: number; avgRating: number; tasksCompleted: number } }> }) => {
    if (active && payload && payload.length) {
      const data = payload[0].payload;
      return (
        <div className="bg-white p-3 border border-gray-200 rounded-lg shadow-lg">
          <p className="font-semibold text-sm mb-1">{data.name}</p>
          <p className="text-xs text-gray-500 mb-2">{data.position} - {data.department}</p>
          <div className="space-y-1">
            <p className="text-sm text-gray-600">
              Tasa de completitud: <span className="font-bold" style={{ color: getBarColor(data.completionRate) }}>
                {data.completionRate}%
              </span>
            </p>
            ...
            <p className="text-sm text-gray-600">
              Total asignadas: <span className="font-medium">{data.assigned}</span>
            </p>
          </div>
  ...
  const avgPerformance = performanceData.length > 0
    ? Math.round(performanceData.reduce((acc, emp) => acc + emp.completionRate, 0) / performanceData.length)
    : 0;
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="text-right space-y-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium">Promedio: {avgPerformance}%</span>
            </div>
            {topPerformer && (
        ...
        <ResponsiveContainer width="100%" height={350}>
          ...
          >
            ...
            <XAxis 
              ...
              domain={[0, 100]}
              tick={{ fontSize: 12 }}
              label={{ value: 'Tasa de Completitud (%)', position: 'insideBottom', offset: -5, style: { fontSize: 12 } }}
            />
            <YAxis 
              type="category" 
              dataKey="name" 
