...
const newRequestSchema = z.object({
  ...
  type: z.enum(["MAINTENANCE", "HOUSEKEEPING", "TOWELS", "ROOM_SERVICE", "CONCIERGE", "TECHNICAL_SUPPORT", "OTHER"]),
...
export default function GuestRequestsInterface({ initialRequests }: GuestRequestsInterfaceProps) {
  ...
  const handleUpdateStatus = async (id: string | number, newStatus: GuestRequest["status"]) => {
    try {
      setLoading(true);
      const numericId = typeof id === 'string' ? parseInt(id) : id;
      const updatedRequest = await guestRequestsApi.update(numericId, { status: newStatus });
      setRequests(prev => 
        prev.map(request => 
                  ...
                  render={({ field }) => (
                    <Select value={field.value} onValueChange={field.onChange}>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="ROOM_SERVICE">Servicio a la habitación</SelectItem>
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <Badge className={`${getStatusColor(request.status)} text-white`}>
                      ...
                      {request.status === "IN_PROGRESS" && <MessageSquare className="mr-1 h-3 w-3" />}
          ...
          {selectedRequest && (
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className={`text-sm ${getPriorityColor(selectedRequest.priority)}`}>
