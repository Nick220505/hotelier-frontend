...
export const cashFlowApi = {
  ...
  getAll: (query: CashFlowQuery = {}): Promise<{ data: CashFlowEntry[]; total: number }> => {
  ...
  update: (id: number, data: Partial<CreateCashFlowEntryDto>): Promise<CashFlowEntry> =>
    apiRequest(`/cash-flow/${id}`, {
      method: 'PATCH',
      body: JSON.stringify(data),
    }),
  ...
  getSummary: (startDate: string, endDate: string): Promise<CashFlowSummary> => {
    const params = new URLSearchParams({
      startDate,
      endDate,
    });
  ...
  getByCategory: (startDate: string, endDate: string): Promise<CashFlowByCategory[]> => {
    const params = new URLSearchParams({
      startDate,
      endDate,
    });
