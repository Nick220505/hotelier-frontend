...
import { reservationsApi, type Reservation as ApiReservation } from "@/lib/api/reservations";
...
export default function Dashboard() {
  ...
  const [monthlyRevenueData, setMonthlyRevenueData] = useState<Array<{ month: string; revenue: number; expenses: number; profit: number }>>([]);
  ...
  const activeReservations = reservations.filter(r => {
    ...
    return (r.status === 'CHECKED_IN' && checkOutDate >= today) ||
  ...
  const monthlyRevenueCop = Number.isFinite(monthlyRevenueRaw) ? monthlyRevenueRaw : 0;
  ...
  const upcomingArrivals = reservations
    .filter(r => r.status === 'CONFIRMED' && new Date(r.checkInDate) >= new Date())
    .slice(0, 5)
    .map(r => ({
      id: r.id.toString(),
  ...
  const upcomingDepartures = reservations
    .filter(r => r.status === 'CHECKED_IN' && new Date(r.checkOutDate) >= new Date())
    .slice(0, 5)
    .map(r => ({
      id: r.id.toString(),
      guestName: r.guestName || 'Sin nombre',
