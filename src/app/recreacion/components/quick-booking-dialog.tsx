...
const quickBookingSchema = z.object({
  ...
  bookingDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "La fecha debe estar en formato YYYY-MM-DD").min(1, "La fecha es requerida"),
  ...
  const onSubmit = async (values: QuickBookingFormValues) => {
    ...
    try {
      ...
      if (values.participants > selectedFacility.capacity) {
        setErrorMessage(`La capacidad máxima es ${selectedFacility.capacity} personas`);
    ...
    <>
            ...
            <div className="text-center space-y-4 w-full">
              ...
              {errorMessage.split('\n').map((part, index) => {
                if (part.includes("puedes reservar")) {
                  return (
                    <div key={index} className="text-amber-600 font-semibold mt-2 p-3 bg-amber-50 rounded-md border border-amber-200">
                      ✅ {part.trim()}
                    </div>
                  );
                }
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-[500px]">
          ...
          <DialogDescription>
            Crea una reserva rápida  para asisitir a nuestras inslalaciones con duración mínima para disponibilidad inmediata
          </DialogDescription>
        </DialogHeader>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <Select
                        value={field.value}
                        onValueChange={(value) => {
                          const selectedGuest = currentReservations.find(r => r.id.toString() === value);
                          if (selectedGuest) {
                            field.onChange(selectedGuest.guestName || "");
                            form.setValue("guestEmail", selectedGuest.guestEmail || "");
                            form.setValue("guestPhone", selectedGuest.guestPhone || "");
                            form.setValue("roomNumber", selectedGuest.room.number || "");
                          }
                        }}
                      >
                        <SelectTrigger>
                          ...
                          <div className="flex items-center justify-between w-full">
                            <div>
                              ...
                              <div className="text-xs text-muted-foreground">
                                Cap: {facility.capacity} • Min: {facility.minimumBookingHours}h
                              </div>
                            </div>
                          </div>
                        </SelectItem>
            ...
            {selectedFacility && (
              <div className="bg-muted p-4 rounded-md space-y-2">
                <h4 className="font-semibold text-sm">Vista previa de la reserva:</h4>
                <div className="grid grid-cols-2 gap-2 text-sm">
                  ...
                  <div>
                    ...
                    <div className="font-medium">{calculateQuickDuration(form.watch("startTime"))}h (mínimo)</div>
                  </div>
                  <div className="col-span-2">
                    <span className="text-muted-foreground">Horario:</span>
                    <div className="font-medium text-amber-600">
                      {selectedFacility.openingTime} a {selectedFacility.closingTime}
                    </div>
                  </div>
                  <div>
                    <span className="text-muted-foreground">Hora fin estimada:</span>
                    <div className="font-medium">
                      {calculateEndTime(form.watch("startTime"), calculateQuickDuration(form.watch("startTime")))}
                    </div>
                  </div>
                </div>
              </div>
