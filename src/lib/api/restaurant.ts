...
const transformRoomServiceOrder = (
  ...
  items: typeof backendOrder.items === 'string' 
    ...
    try {
      ...
      // If it's a string, try to parse it as JSON
        ...
        if (backendItem.ingredients.trim().startsWith('[')) {
          ...
          ingredients = backendItem.ingredients.split(',').map(item => item.trim());
        }
      }
    } catch (error) {
      console.warn('Failed to parse ingredients:', backendItem.ingredients, error);
  ...
  updateMenuItem: async (
    ...
    try {
      ...
      if (item.backendId) {
        ...
        console.log("Backend ID not available, fetching menu items to find backend ID for:", item.id);
        const items = (await apiRequest(
          "/restaurant/menu-items",
        )) as BackendMenuItem[];
        
        const foundItem = items.find((i: BackendMenuItem) => i.itemCode === item.id);
      ...
      const backendItem = (await apiRequest(`/restaurant/menu-items/${backendId}`, {
        method: "PUT",
        body: JSON.stringify(updateData),
      })) as BackendMenuItem;
